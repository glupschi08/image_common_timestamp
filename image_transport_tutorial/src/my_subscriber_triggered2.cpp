...
{
  ros::init(argc, argv, "image_listener_triggered2");
  ros::NodeHandle nh;
  cv::namedWindow("view_triggered2", 400);//CV_WINDOW_AUTOSIZE
  //waitKey();
  cv::startWindowThread();
  image_transport::ImageTransport it(nh);
  image_transport::Subscriber sub = it.subscribe("avt_camera_img2", 1, imageCallback);
  ros::spin();
  cv::destroyWindow("view_triggered2");
}
