...
{
  ros::init(argc, argv, "image_listener_multi");
  ros::NodeHandle nh;
  cv::namedWindow("view_multi", 400);//CV_WINDOW_AUTOSIZE
  //waitKey();
  cv::startWindowThread();
  image_transport::ImageTransport it(nh);
  image_transport::Subscriber sub_1 = it.subscribe("cam_1/image_raw", 1, imageCallback_1);
  image_transport::Subscriber sub_2 = it.subscribe("cam_2/image_raw", 1, imageCallback_2);
  ros::spin();
  cv::destroyWindow("view_multi");
}
